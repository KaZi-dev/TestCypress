...
export class TestServiceService {
  ...
  public parse() {
    this.teststr = '{ "variables": {"name": "' + this.name + '", "address": "' + this.address + '"}}';
  ...
  public complUserTask(): Observable<any> {
    this.http.get('http://localhost:8080/engine-rest/task?name=Test User&processInstanceId=' + this.processID2, {observe: 'body'})
      .subscribe(
      result => {
    ...
    return this.http.get('http://localhost:8080/engine-rest/task?name=Test User&processInstanceId' + this.processID2);
  }
